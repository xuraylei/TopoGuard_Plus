...
class Port {
	...
	// naive version of hash computation
	@Override
	public int hashCode() {
		int hash = 3;
...
public class TopoloyUpdateChecker implements IDeviceListener,
	...
	//public ControlLinkProber linkProber;
	...
	public void deviceMoved(IDevice device) {
		...
		if (previousLocation.length == 1) { // only one location in AP history
			...
			if (hosts.get(src_mac) == false) { // indicating the host is not disabled	 
				...
				//logger.warn("Violation: Host Move from switch {} port {} without Port ShutDown", previousDpid, previousPortID);
			}
	...
	public void startUp(FloodlightModuleContext context)
		...
		//floodlightProvider.addOFSwitchListener(this.linkProber);
